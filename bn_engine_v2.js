...
(function(){
  ...
  function bnStorageKey(company){
    return 'itmen_bn_picks::' + String(company||'').trim().toLowerCase();
  }

  function normalizeBnRows(raw){
    if(!raw) return [];
    ...
    if(typeof raw === 'object'){
      if(Array.isArray(raw.bns)) return raw.bns;
      if(Array.isArray(raw.items)) return raw.items;
    }
    return [];
  }

  function hydratePicksFromRows(rows){
    const next = {};
    normalizeBnRows(rows).forEach((row)=>{
      if(!row || typeof row !== 'object') return;
      const needId = String(row.bn_id || row.need_id || row.id || '').trim();
      if(!needId) return;
      next[needId] = {
        causes: new Set(Array.isArray(row.causes) ? row.causes.filter(Boolean) : []),
        pains: new Set(Array.isArray(row.pains) ? row.pains.filter(Boolean) : []),
        answers: Array.isArray(row.answers) ? row.answers.map(v=> String(v ?? '')) : []
      };
    });
    return next;
  }

  function hydratePicksFromPayload(payload){
    if(!payload || typeof payload !== 'object') return {};
    const candidates = [
      payload.bns,
      payload.bn_payload,
      payload.bnPayload,
      payload.bn_log_payload,
      payload.bnLogPayload,
      payload.business_needs,
      payload.businessNeeds,
      payload.bn_state,
      payload.bnState
    ];
    for(const candidate of candidates){
      const picks = hydratePicksFromRows(candidate);
      if(Object.keys(picks).length) return picks;
    }
    return {};
  }

  function loadPicksFromLocal(company){
    try{
      const raw = localStorage.getItem(bnStorageKey(company));
      if(!raw) return {};
      return hydratePicksFromRows(JSON.parse(raw));
    }catch(_){ return {}; }
  }

  function savePicksToLocal(company, payload){
    try{
      if(!company) return;
      const rows = Array.isArray(payload?.bns) ? payload.bns : [];
      localStorage.setItem(bnStorageKey(company), JSON.stringify(rows));
    }catch(_){ }
  }
  ...
  async function loadCompany({company, row}){
    ...
    try{
      ...
      // Restore saved BN selections if they are present in the loaded payload;
      // otherwise fall back to last locally cached BN snapshot for this company.
      const restoredFromPayload = hydratePicksFromPayload(payload);
      const restoredFromLocal = Object.keys(restoredFromPayload).length ? {} : loadPicksFromLocal(name || company);
      BN_STATE.picks = Object.keys(restoredFromPayload).length ? restoredFromPayload : restoredFromLocal;
      SELECTED_THEME = '';
      SELECTED_NEED_ID = '';
  ...
  async function saveAll(themeScores){
    try{
      ...
      savePicksToLocal(payload.company_name, payload);
