...
(function(){
  ...
  function computeHeatmapFromRow(row){
    ...
    function handle(prefix, count){
      for(let i=1;i<=count;i++){
        ...
        const scoreRaw = row[`${prefix}_${idx}_score`];
        const v = (scoreRaw===''||scoreRaw==null) ? null : Number(scoreRaw);
        if(!label || v===null || Number.isNaN(v)) continue;
  ...
  function renderNeeds(){
    ...
    const bnList = (grouped[theme] || []).filter(bn=> strengthForBN(bn) > 0);
    ...
    if(!top.length){
      box.innerHTML = `<div class="small">По тематике «${esc(theme)}» бизнес‑потребности не активированы (риск низкий или нет ответов интервью).</div>`;
    ...
    box.innerHTML = top.map(bn=>{
      ...
      return `<button class="needBtn${active}" data-bn="${esc(bn.id)}">
        ...
        <div class="needMeta">Сила: <b>${Math.round(strengthForBN(bn))}</b> · Вес: ${esc(bn.weight)} · Зона: ${esc(bn.zone||'—')}</div>
      </button>`;
    }).join('');
